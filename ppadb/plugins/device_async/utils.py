...
class Utils(PluginAsync):
    async def get_top_activity(self):
        activities = await self.get_top_activities()
        if activities:
            return activities[0]
        else:
            return None

    async def get_top_activities(self):
    ...
    async def get_meminfo(self, package_name):
    ...
    async def get_pid(self, package_name, toybox=False):
        ...
        for cmd in cmds:
            result = await self.shell(cmd.format(package_name))
            if result:
                break
    ...
    async def get_uid(self, package_name):
        cmd = 'dumpsys package {} | grep userId'.format(package_name)
        result = await self.shell(cmd).strip()
