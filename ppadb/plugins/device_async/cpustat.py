...
class CPUStat(PluginAsync):
    total_cpu_pattern = re.compile(
        "cpu\s+([\d]+)\s([\d]+)\s([\d]+)\s([\d]+)\s([\d]+)\s([\d]+)\s([\d]+)\s([\d]+)\s([\d]+)\s([\d]+)\s")

    async def cpu_times(self):
        return await self.get_total_cpu()

    async def cpu_percent(self, interval=1):
        cpu_times_start = await self.cpu_times()
        time.sleep(interval)
        cpu_times_end = await self.cpu_times()

        diff = cpu_times_end - cpu_times_start
        return round(((diff.user + diff.system) / diff.total()) * 100, 2)

    async def cpu_count(self):
        result = await self.shell('ls /sys/devices/system/cpu')
        match = re.findall(r'cpu[0-9+]', result)
        return len(match)

    async def get_total_cpu(self):
        result = await self.shell('cat /proc/stat')
    ...
    async def get_pid_cpu(self, pid):
        result = await self.shell('cat /proc/{}/stat'.format(pid)).strip()
    ...
    async def get_all_thread_cpu(self, pid):
        result = await self.shell("ls /proc/{}/task".format(pid))
